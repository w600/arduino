...
#define ADC_DEST_BUFFER_DMA     		(uint32_t)0x20028000//(uint32_t)0x20037000	//前面的地方高速SPI可能会用	
...
#define CONFIG_ADC_VCM(n)				(((uint32_t)n&0x3F)<<26)
...
typedef struct adc_st{
	uint8_t dmachannel;
	void (*adc_cb)(uint16_t *buf, uint16_t len);
	void (*adc_bigger_cb)(uint16_t *buf, uint16_t len);
	void (*adc_dma_cb)(uint16_t *buf,uint16_t len);
	uint16_t valuelen;		/*dma 采样数据长度*/
	uint16_t offset;
}ST_ADC;
...
**********************************************************************************************************/
void tls_adc_init(uint8_t ifusedma,uint8_t dmachannel);
...
* 		
**********************************************************************************************************/
void tls_adc_irq_register(int inttype, void (*callback)(uint16_t *buf, uint16_t len));
...
void tls_adc_temp_offset_with_cpu(uint8_t calTemp12);
...
* Returns    : 
**********************************************************************************************************/
uint16_t tls_read_adc_result(void);
...
void signedToUnsignedData(uint16_t *adcValue, uint16_t *offset);
void tls_adc_buffer_bypass_set(uint8_t isset);
void tls_adc_cmp_start(int Channel, int cmp_data, int cmp_pol);

uint8_t  adc_get_offset(void);
uint32_t adc_get_interTemp(void);
uint16_t adc_get_inputVolt(uint8_t channel);
uint16_t adc_get_interVolt(void);
uint32_t adc_temp(void);
